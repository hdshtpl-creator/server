...
> **⚠️ Bản trùng cũ.** Tài liệu chính thức là [`README.md`](README.md); triển khai
> lên máy chủ (frontend + tên miền) xem [`../deploy/README.md`](../deploy/README.md).
> File này giữ lại để tham khảo, một số lệnh đã lỗi thời (ví dụ "để mặc định lần đầu"
> — mã mới từ chối `JWT_SECRET` mẫu).
...
nano .env                         # sửa mật khẩu (hoặc để mặc định lần đầu)
...
Đổi `JWT_SECRET` trong `.env` sang chuỗi ngẫu nhiên dài trước khi chạy thật.

## ⚠️ Trước khi công khai ra internet

Còn lại: HTTPS (Nginx + Let's Encrypt cho app.hdslaw.vn), rate limit theo gói.
Xác thực JWT đã xong. Để dev chính làm phần hạ tầng mạng.
