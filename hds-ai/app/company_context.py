...
def staff_person_index(dept_ids=None, is_banqt=False, can_finance=False):
    """{tên bộ hồ sơ: [(mã tài liệu, tiêu đề)…]} — ngăn '8. HỒ SƠ NHÂN SỰ'.

    Một chỗ duy nhất trả lời hai câu hỏi khác nhau: "có bao nhiêu bộ" (đếm) và
    "bộ của Mai gồm những giấy tờ nào" (ghim nguồn khi tra cứu). Tách hai truy
    vấn là sớm muộn hai nơi lệch nhau về điều kiện duyệt/quyền. RLS của phiên
    áp như thường nên mỗi người chỉ thấy phần mình được xem.
    """
    try:
        with db.session(role="internal", dept_ids=dept_ids, is_banqt=is_banqt,
                        can_finance=can_finance) as conn:
            with conn.cursor() as cur:
                cur.execute("""SELECT person_folder, id, title
                                 FROM documents
                                WHERE doc_type='ho_so_ns' AND person_folder IS NOT NULL
                                  AND approved AND label_verified
                                  AND coalesce(active,true)
                                ORDER BY person_folder, title, id""")
                index: dict[str, list[tuple]] = {}
                for person, doc_id, title in cur.fetchall():
                    index.setdefault(person, []).append((doc_id, title))
                return index
    except Exception:
        return {}


def person_files_block(index, persons) -> str:
    """Khối DỮ LIỆU CÔNG TY liệt kê ĐỦ giấy tờ trong bộ hồ sơ của người được hỏi.

    Retrieval chỉ đưa vào prompt những đoạn khớp nhất, nên model không biết bộ
    hồ sơ còn giấy tờ nào khác — hỏi "chi tiết Mai" mà bot chỉ kể được 2 trong
    12 giấy tờ thì người dùng tưởng kho chỉ có bấy nhiêu. Danh mục ở đây lấy
    thẳng bằng SQL nên luôn đủ và luôn đúng, đúng nguyên tắc "cây là nguồn sự
    thật về số lượng, chi tiết thì phải vào đọc trong bộ".
    """
    lines = []
    for name in persons or []:
        files = (index or {}).get(name) or []
        if not files:
            continue
        lines.append(f"### BỘ HỒ SƠ NHÂN SỰ CỦA {name} — {len(files)} giấy tờ "
                     f"đã duyệt trong kho (tính đến {date.today()}):")
        lines += [f" · {title}" for _, title in files]
    if not lines:
        return ""
    lines.append("Danh mục trên là ĐỦ các giấy tờ hiện có của người này trên cây "
                 "thư mục. Nội dung chi tiết nằm ở phần TÀI LIỆU THAM KHẢO bên "
                 "dưới; giấy tờ nào không có đoạn trích kèm theo thì nêu rõ là "
                 "chưa đọc được nội dung của giấy tờ đó, không suy đoán.")
    return "\n".join(lines)


def _tree_staff(dept_ids=None, is_banqt=False, can_finance=False):
    """[(tên bộ hồ sơ, số giấy tờ đã duyệt), …] — dùng cho câu ĐẾM nhân sự."""
    return [(person, len(ids)) for person, ids
            in staff_person_index(dept_ids, is_banqt, can_finance).items()]


# Tín hiệu cho thấy câu hỏi đang nói tới HỒ SƠ MỘT CON NGƯỜI. Tên riêng tiếng
# Việt một tiếng (Mai, Ngân, Nhi, Thu…) trùng với từ thông thường, nên chỉ khớp
# tên khi câu có ít nhất một tín hiệu như dưới đây. Cố ý KHÔNG đưa "hop dong"
# trần vào: "hợp đồng ngân hàng" sẽ biến thành hồ sơ của chị Ngân.
PERSON_CONTEXT_WORDS = {
    "chi tiet", "thong tin", "ho so", "giay to", "so yeu", "ly lich", "li lich",
    "can cuoc", "cccd", "cmnd", "bang cap", "bang tot nghiep", "chung chi",
...
_TIME_PHRASES = ("ngay mai", "sang mai", "chieu mai", "toi mai", "mai mot",
                 "hom nay", "hom qua", "tuan sau", "thang sau")


def detect_staff_person(question, names) -> list[str]:
    """Tên các bộ hồ sơ nhân sự được nhắc ĐÍCH DANH trong câu hỏi.

    Hàm thuần (nhận sẵn danh sách tên) để test không cần CSDL.

    Vì sao cần: câu "chi tiết Mai" hỏi về MỘT CON NGƯỜI, không phải hỏi quân
    số. Không nhận ra điều đó thì router coi nó là câu nhân sự nói chung và trả
    lại đúng bảng đếm vừa hiện ở lượt trước (ca thật 21/08/2026) — người dùng
    hỏi mà nhận lại y nguyên câu trả lời cũ, tưởng bot hỏng.
    """
    q = _fold(question)
    ...
    for name in names or []:
        ...
        if not name_tokens:
            continue
        for start in range(len(tokens) - len(name_tokens) + 1):
            if tokens[start:start + len(name_tokens)] != name_tokens:
                continue
...
def structured_answer(question, channel, client_id=None, dept_ids=None,
    ...
    `person_index` (tên bộ hồ sơ → mã tài liệu) do rag.prepare truyền xuống để
    khỏi truy vấn hai lần cùng một thứ; None thì tự lấy khi cần.
    """
    if channel != "internal":
        return None
    ...
    if intent == "account_directory":
        # Chỉ đọc bảng users khi thật sự hỏi về TÀI KHOẢN. Gọi vô điều kiện như
        # trước là mỗi câu hỏi nhân sự phải cõng thêm một truy vấn không dùng.
    ...
    tree_index = None
    if intent in ("staff_directory", "employment_contract"):
        tree_index = (person_index if person_index is not None
                      else staff_person_index(dept_ids, is_banqt, can_finance))
        # Câu nêu ĐÍCH DANH một người ("chi tiết Mai", "HĐLĐ của Ngân") hỏi về
        # MỘT CON NGƯỜI, không phải quân số — trả bảng đếm là trả lạc đề, và
        # nếu vừa trả bảng đó ở lượt trước thì người dùng nhận lại y nguyên câu
        # cũ (ca thật 21/08/2026). Trả None để rơi xuống tra cứu: rag.prepare
        # ghim đúng bộ hồ sơ của người đó rồi model đọc nội dung mà trả lời.
        if detect_staff_person(question, list(tree_index or {})):
            return None
    ...
    if not employees and intent == "staff_directory":
        ...
        #
        # NHƯNG bảng này chỉ ĐẾM. Hỏi tiếp đòi CHI TIẾT ("chi tiết từng người",
        # "họ làm gì") mà trả lại đúng bảng vừa hiện là câu trả lời chết: người
        # dùng thấy bot lặp lại chính nó. Lúc đó nhường cho tra cứu đọc nội
        # dung trong các bộ hồ sơ — khác nhánh sổ employees, vốn tự liệt kê
        # được chức danh/phòng ban nên bảng của nó đã là chi tiết.
        if (state or {}).get("intent") == "staff_directory" and _detail_intent(
                _fold(question)):
            return None
        rows = [(person, len(ids)) for person, ids in (tree_index or {}).items()]
        if rows:
